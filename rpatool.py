...
class RenPyArchive:
    ...
    def get_version(self):
        ...
        if magic.startswith(self.SVAC1_MAGIC):
            parts = magic.split()
            if len(parts) == 4:
                return 4      # true SVAC-1.0 archive
            else:
                return 3    # false SVAC, actually RPA-3.0 with a custom header
        elif magic.startswith(self.RPA3_2_MAGIC):
            return 3.2
        elif magic.startswith(self.RPA3_MAGIC):
            return 3
        elif magic.startswith(self.RWA3_MAGIC):
            return 3
    ...
    def load(self, filename):
        ...
        if self.version in [1, 2, 3, 3.2]:
            self.indexes = self.extract_indexes()
        elif self.version == 4:
            self.indexes = self.extract_svac1_indexes()
        else:
            raise ValueError('unsupported Ren\'Py archive version')

    # Extract file indexes from opened SVAC-1.0 archive.
    def extract_svac1_indexes(self):
        import json, zlib

        self.handle.seek(0)
        header = self.handle.readline().decode('utf-8')
        parts = header.split()

        # parts[1] = offset hex
        index_offset = int(parts[1], 16)

        # Go to the OggS block containing the compressed JSON data,
        # read it and try to decode it as zlib first, if that fails,
        # decode it as Ogg encapsulated Vorbis comments.
        self.handle.seek(index_offset)
        compressed = self.handle.read()

        try:
            # Variant B: Direct zlib stream, no Ogg encapsulation
            json_data = zlib.decompress(compressed).decode("utf-8")
        except:
            # Variant A: Encapsulated Ogg stream, decode it to extract the JSON from Vorbis comments
            json_data = self.decode_svac1_ogg(compressed)

        # Load JSON data into a Python dict
        raw = json.loads(json_data)

        # Convert to internal index format
        indexes = {}
        for name, info in raw["files"].items():
            offset, length = info
            indexes[name] = [(offset, length)]

        return indexes

    def decode_svac1_ogg(self, data):
        # Search for packet type 3 “vorbis”, which contains the comments with the JSON data.
        marker = b"\x03vorbis"
        pos = data.find(marker)
        if pos == -1:
            raise ValueError("SVAC-1.0: Vorbis package (type 3) not found in Ogg stream")

        # After the marker, there is a “vendor_length” field (4 little-endian bytes).
        vendor_len = int.from_bytes(data[pos+7:pos+11], "little")

        # Skip the vendor string (vendor_length bytes) to get to the comment list.
        comment_start = pos + 11 + vendor_len

        # Read the number of comments (4 bytes LE)
        comment_count = int.from_bytes(data[comment_start:comment_start+4], "little")
        p = comment_start + 4

        # Browse Vorbis comments to find the one starting with "JSON=", which contains the JSON data.
        for _ in range(comment_count):
            length = int.from_bytes(data[p:p+4], "little")
            p += 4
            comment = data[p:p+length]
            p += length

            # The JSON is in a comment.
            if comment.startswith(b"JSON="):
                return comment[5:].decode("utf-8")

        raise ValueError("SVAC-1.0: JSON not found in Vorbis comments.")
